// ─────────────────────────────────────────────────────────────────────────────
// Approval routes. This module is the ONLY code wired to teams/send.js
// (via server.js: ctx.sendTeamsMessage). No other code path can send to Teams.
// ─────────────────────────────────────────────────────────────────────────────
import { createDraftForMessage } from '../teams/webhook.js';
...
export async function sendApprovedDraft(ctx, draft, { edited }) {
  ...
  // 2) Send via Graph. ctx.sendTeamsMessage is the ONLY send path and is
  //    set exclusively here (see server.js); tests inject a mock.
  try {
    const sent = await ctx.sendTeamsMessage(draft.chat_id, text);
    // 3) Mark sent + record as my message + style example.
    draftRepo.markSent(draftId);
    if (sent?.id) {
      messageRepo.save({
    ...
    ctx.logger.info({ draftId }, 'message send succeeded');
    return { ok: true, message: `Reply sent to Teams (draft #${draftId}).` };
  } catch (err) {
    draftRepo.markFailed(draftId, err.message);
    ctx.logger.warn({ draftId, errStatus: err.status }, 'message send failed');
